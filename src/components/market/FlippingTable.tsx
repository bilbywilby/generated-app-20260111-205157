      ...
      <Table>
        <TableHeader className="bg-stone-950/50">
          <TableRow className="border-stone-800 hover:bg-transparent">
            ...
            <TableHead className="text-[10px] uppercase font-bold text-stone-500 h-10 text-right cursor-pointer" onClick={() => handleSort('alch')}>
               Alch Profit <ArrowUpDown className="inline w-3 h-3 ml-1" />
            </TableHead>
        ...
        <TableBody>
          {sortedData.length > 0 ? (
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className={cn(
                          ...
                          row.alch.profit > 0 ? "text-emerald-500" : "text-stone-600"
                        )}>
                          {row.alch.profit.toLocaleString()}
                          <Info className="w-2.5 h-2.5 opacity-50" />
                        </span>
                      </TooltipTrigger>
                      <TooltipContent className="bg-stone-900 border-stone-800 text-[10px]">
                        High Alch - Buy (Low) - Nature Rune (~{naturePrice} gp)
                      </TooltipContent>
                    </Tooltip>
                    <div className={cn("text-[9px] opacity-60", row.alch.roi > 0 ? "text-emerald-500" : "text-stone-600")}>
            ...
            <TableRow>
              <TableCell colSpan={7} className="h-24 text-center text-xs text-stone-600">
