...
export function ItemDetailSheet({ prices }: { prices: Record<string, any> }) {
  ...
  const { data: history, isLoading } = useQuery({
    queryKey: ['timeseries', selectedId],
    queryFn: () => selectedId ? fetchTimeSeries(selectedId) : null,
    enabled: !!selectedId,
  });
    ...
    <Sheet open={!!selectedId} onOpenChange={(open) => !open && setSelectedId(null)}>
      <SheetContent className="sm:max-w-xl bg-stone-950 border-stone-800 text-stone-200 overflow-y-auto">
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-semibold text-stone-400 uppercase tracking-wider">Price History (24h)</h3>
